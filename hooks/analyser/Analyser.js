...
export default class Analyser {
    ...
    getLowerMax() {
        ...
        const sum = lowerHalfArray.reduce((partialSum, a) => partialSum + a, 0);
        //return Math.max(...this.getLowerHalfArray()) / lowerHalfArray.length;
        const maxNumber = Math.max(...this.getLowerHalfArray());
        if(maxNumber === 0.0) {
            return 0.0;
        }
        const res = (maxNumber/sum)*100;
        return res;
    }
